...
ループの動きと連動したフラグを使って、フラグが既に立っていたらイベント発火時の処理を場合により`return`するなど、なにかしらの工夫が必要な場面も考えられます。
...
torna-do では爆発エフェクトの演出などに利用しているノイズテクスチャは、画像としてロードするのではなく、プロシージャルに生成して利用しています。
